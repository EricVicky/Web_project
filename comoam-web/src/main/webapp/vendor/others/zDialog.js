 ...
 * 最后修正：2009-12-18
 **/
var IMAGESPATH = 'vendor/others/images/'; //图片路径配置
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" class="btn btn-info" value="View" id="_ButtonOK_' + this.ID + '"/>\
                &nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="btn btn-info" value="Close" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
...
Dialog.prototype.autoClose = function () {
    ...
    this.AutoClose -= 1;
    topWin.$id("_Title_" + this.ID).innerHTML = "close automatically after "+this.AutoClose + " seconds";
    if (this.AutoClose <= 0) {
        this.close();
    } else {
        var self = this;
